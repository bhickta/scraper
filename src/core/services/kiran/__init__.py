"""
GenericMCQExtractor — configurable MCQ extractor for Kiran-style PDFs.
"""
...
class GenericMCQExtractor(MCQExtractor):
    """MCQ extractor with configurable regex patterns for questions and answers."""

    def __init__(self, pdf_service: PDFService, **kwargs):
        super().__init__(pdf_service=pdf_service, **kwargs)
        self.questions = {}
        self.explanations = {}
        self.mcqs = []
        self.text = ""
    ...
    # ------------------------------------------------------------------
    # Pattern hooks — override in subclasses for different formats
    # ------------------------------------------------------------------
    ...
    def process_questions(self) -> None:
        ...
        for mcq in mcqs:
            ...
            question_text = self._remove_options_from_text(question_text).strip()
    ...
    def process_explanation(self) -> None:
    ...
    def _extract_options(self, question_text: str) -> list:
        return self.get_options_pattern().findall(question_text)

    def _remove_options_from_text(self, question_text: str) -> str:
        return self.get_options_pattern().sub("", question_text)

    def _get_explanation_content(self, match) -> str:
        start = match.end()
    ...
    def _find_next_explanation(self, match):
        return self.get_explanation_pattern().finditer(self.text[match.end():])

    def get_mcqs(self, source_name: str) -> None:
        self.mcqs = [
            {
                ...
                "d": question.get("d", ""),
                "source": source_name,
                "answer": self.explanations.get(qno, {}).get("answer"),
                "explanation": self.explanations.get(qno, {}).get("explanation"),
            }
            for qno, question in self.questions.items()
        ]

    def run(self, source_name: str, **kwargs):
...
def main(pdf_path: str, output_path: str, pages, source_name: str) -> None:
